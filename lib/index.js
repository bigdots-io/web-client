...
export function updateDot({ y, x, hex }) {
...
export function BigdotsClient({ config, }) {
    useEffect(() => {
        createDisplayEngine(Object.assign(Object.assign({}, config), { onPixelChange: updateDot }));
    }, []);
    const { height, width } = config.dimensions;
    var adjustedBrightness = (50 + 100 / 2) / 100;
    return (_jsx("div", Object.assign({ style: { background: "#000" } }, { children: [...Array(height).keys()].map((y) => (_jsx(Row, Object.assign({ y: y, opacity: adjustedBrightness }, { children: [...Array(width).keys()].map((x) => (_jsx(Column, Object.assign({ y: y, x: x }, { children: _jsx(Dot, { y: y, x: x }) })))) }), `row_${y}`))) })));
}
