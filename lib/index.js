...
export default function BigdotsDisplay({ layers, dimensions, }) {
    ...
    return (_jsx("div", Object.assign({ ref: ref, style: { background: "#000" } }, { children: [...Array(height).keys()].map((y) => (_jsx(Row, Object.assign({ y: y, opacity: adjustedBrightness }, { children: [...Array(width).keys()].map((x) => (_jsx(Column, Object.assign({ y: y, x: x }, { children: _jsx(Dot, { y: y, x: x }) }), `row_${y}_col_${x}`))) }), `row_${y}`))) })));
}
